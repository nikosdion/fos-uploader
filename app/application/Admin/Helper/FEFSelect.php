...
abstract class FEFSelect
{
	...
	/**
	 * Returns a published / unpublished selector with a no selection option
	 *
	 * @param   string $name     The value of the HTML name attribute
	 * @param   array  $attribs  Additional HTML attributes for the `<select>` tag
	 * @param   string $selected The key that is selected, empty for no selection
	 * @param   string $yes      Language key for Yes
	 * @param   string $no       Language key for No
	 * @param   string $id       The id for the field or null for no id
	 *
	 * @return string
	 */
	public static function published(string $name, array $attribs = [], ?string $selected = null,
	                                 string $yes = 'AWF_PUBLISHED', string $no = 'AWF_UNPUBLISHED',
	                                 ?string $id = null): string
	{
		$noneText = '&emdash;&emdash;&emdash;';

		if (isset($attribs['placeholder']))
		{
			$noneText = '-- ' . Text::_($attribs['placeholder']) . ' --';
		}

		$arr = [
			static::option('', $noneText),
			static::option('0', Text::_($no)),
			static::option('1', Text::_($yes)),
		];

		return static::genericlist($arr, $name, $attribs, 'value', 'text', (string) $selected, $id, false);
	}
	 ...
	 * @param   string $yes      Language key for Yes
	 * @param   string $no       Language key for no
	 * @param   mixed  $id       The id for the field or null for no id
	 *
	 * @return  string  HTML for the radio list
	 */
	public static function booleanlist(string $name, array $attribs = [], ?int $selected = null,
	                                   string $yes = 'AWF_YES', string $no = 'AWF_NO', ?string $id = null): string
	{
		$arr = [static::option('0', Text::_($no)), static::option('1', Text::_($yes))];

		return static::radiolist($arr, $name, $attribs, 'value', 'text', $selected ? '1' : '0', $id);
	}
	...
	public static function genericlist(array $data, string $name, ?array $attribs = null, string $optKey = 'value',
	                                   string $optText = 'text', ?string $selected = null, ?string $idtag = null,
	                                   bool $translate = false): string
	{
		// Set default options
		$options = array_merge(Html::$formatOptions, ['format.depth' => 0, 'id' => false]);
	...
	{
		// Set default options
		$options = array_merge(Html::$formatOptions, ['format.depth' => 0, 'option.format' => '', 'id' => null]);
	 ...
	 * @return  string  HTML for the select list
	 */
	public static function options(array $arr, $optKey = 'value', string $optText = 'text', ?string $selected = null,
	                               bool $translate = false): string
	{
		$options = array_merge(
			Html::$formatOptions,
	 ...
	 * @return  string  HTML for the select list
	 */
	public static function radiolist(array $data, string $name, ?array $attribs = null, $optKey = 'value',
	                                 string $optText = 'text', ?string $selected = null, ?string $idtag = null,
	                                 bool $translate = false): string
	{
